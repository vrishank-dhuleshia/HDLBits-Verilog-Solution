module top_module(
    ...
    assign next_state[1] = (state[0] & in) | (state[9] & in) | (state[8] & in);   // S0 -> S1
    ...
    assign next_state[7] = (state[6] & in) | (state[7] & in);  // S6->S7, S7 self-loop
    assign next_state[8] = state[5] & ~in;  // S5 -> S8
    assign next_state[9] = (state[6] & ~in); // S6->S9, S8->S9
